...
//   var blur = document.getElementById('blur'); // Get the overlay element
//   blur.classList.remove('active'); // Remove the 'active' class from the overlay
// }
...
function toggle(){
  ...
  var popup = document.getElementById('form-add-meal');
...
closeModalButtons.forEach(button => {
  button.addEventListener('click', () => {
    const modal = button.closest('.form-add-meal');
    closeModal(modal);
  });
});
