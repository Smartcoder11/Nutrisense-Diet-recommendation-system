document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentDate = new Date().toISOString().split('T')[0].split('-').reverse().join('-'); // Format date in dd-mm-yyyy
    const uid = document.getElementById('uid').value; // Fetch uid from wherever it's available

    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            if (this.checked) {
                const mealData = this.previousElementSibling.value.split(',');
                console.log(mealData);
                // Verify meal data in the console

                // Send meal data to Flask endpoint
                fetch('/update_meal', {
                    method: 'POST',
                    headers: {
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        throw new Error('Failed to add meal'); // Throw an error for non-successful response
                    }
                })
                .then(data => {
                    ...
                    // Hide the container
                    this.closest('.meal-container').style.display = 'none';
                })
                .catch(error => {
